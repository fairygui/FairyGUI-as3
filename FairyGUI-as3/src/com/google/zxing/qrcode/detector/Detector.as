...
package com.google.zxing.qrcode.detector
{
	...
	public class Detector
	{
		...
		public function processFinderPatternInfo(info:FinderPatternInfo):DetectorResult
		{
			...
			var transform:PerspectiveTransform = createTransform(topLeft, topRight, bottomLeft, alignmentPattern, dimension);
		...
		private static function sampleGrid(image:BitMatrix, transform:PerspectiveTransform, dimension:int):BitMatrix
		{
		 ...
		 */
		private static function computeDimension(topLeft:ResultPoint, topRight:ResultPoint, bottomLeft:ResultPoint, moduleSize:Number):int
		{
// note : check custom round function at the bottom
		...
		{
			var moduleSizeEst1:Number = sizeOfBlackWhiteBlackRunBothWays(int(pattern.getX()), int(pattern.getY()), int(otherPattern.getX()), int(otherPattern.getY()));
			var moduleSizeEst2:Number = sizeOfBlackWhiteBlackRunBothWays(int(otherPattern.getX()), int(otherPattern.getY()), int(pattern.getX()), int(pattern.getY()));
		...
		private function sizeOfBlackWhiteBlackRun(fromX:int, fromY:int, toX:int, toY:int):Number
		{
			...
			for (var x:int = fromX, y:int = fromY; x != xLimit; x += xstep)
			{
				...
				if (((state == 0) && !pixl) || ((state == 1) && pixl) || ((state == 2) && !pixl))
				{
					if (state == 2)
					{
		 ...
		 */
		private function findAlignmentInRegion(overallEstModuleSize:Number, estAlignmentX:int, estAlignmentY:int, allowanceFactor:Number):AlignmentPattern
		{
			...
			var alignmentFinder:AlignmentPatternFinder = new AlignmentPatternFinder(image, alignmentAreaLeftX, alignmentAreaTopY, alignmentAreaRightX - alignmentAreaLeftX, alignmentAreaBottomY - alignmentAreaTopY, overallEstModuleSize, resultPointCallback);
		...
		public static function createTransform(topLeft:ResultPoint, topRight:ResultPoint, bottomLeft:ResultPoint, alignmentPattern:ResultPoint, dimension:int):PerspectiveTransform
		{
