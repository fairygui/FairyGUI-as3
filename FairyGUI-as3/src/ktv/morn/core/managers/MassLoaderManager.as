...
package ktv.morn.core.managers {
	...
	public class MassLoaderManager extends EventDispatcher {
		...
		private static var instance:MassLoaderManager;
		public function MassLoaderManager() {
			init();
		}
		
		public static function getInstance():MassLoaderManager
		{
			if(!instance)
			{
				instance=new MassLoaderManager();
			}
			return instance;
		}
		...
		public function load(url:String, type:uint, priority:uint = 1, complete:Handler = null, progress:Handler = null, error:Handler = null, isCacheContent:Boolean = true):void {
		...
		private function endLoad(resInfo:ResInfo, content:*):void {
			...
			if (content == null) {
				...
				if (errorCount < _retryNum) {
					...
					LogManager.log.warn("mass load error:", resInfo.url);
		...
		public function loadAssets(arr:Array, complete:Handler = null, progress:Handler = null, error:Handler = null, isCacheContent:Boolean = true):void {
			...
			for (var i:int = 0; i < itemCount; i++) {
				...
				if (item is String) {
					item = {url: item, type: ResLoader.checkType(String(item)), size: 1, priority: 1};
				}
				item.progress = 0;
				totalSize += item.size;
				items.push(item);
